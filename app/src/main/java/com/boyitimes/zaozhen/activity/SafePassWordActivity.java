...
public class SafePassWordActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setAndroidNativeLightStatusBar(SafePassWordActivity.this, true);
            ...
            public void onClick(View v) {
                if (!"".equals(edt_mima.getText().toString()) && edt_mima.getText().toString().length() > 6) {
                    if (!edt_mima.getText().toString().equals(edt_mimas.getText().toString())) {
                        Toast.makeText(SafePassWordActivity.this, "两次密码不一致", Toast.LENGTH_LONG).show();
                    } else {
                        LoadingDialogUtils.closeDialog(mDialog);
                        new HttpTask().execute();
                    }
                } else {
                    Toast.makeText(SafePassWordActivity.this, "请输入正确密码", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
...
//            textView.setText("loading..." + progresses[0] + "%");
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                if ("2000".equals(code)) {
                    BysdApplication.isXiuGai = true;
                    Toast.makeText(SafePassWordActivity.this, description, Toast.LENGTH_SHORT).show();
                    hideKeyBoard(SafePassWordActivity.this, edt_mima);
                    finish();
                } else {
                    Toast.makeText(SafePassWordActivity.this, description, Toast.LENGTH_SHORT).show();
                }
